    ...
    MarketRegistry
{
    ...
    function _fillOrder(
        ...
        _assertFillable(order, orderInfo, takerAddress, signature);
        ...
        returns (LibOrder.OrderInfo memory orderInfo)
    {
        ...
        orderInfo.orderHash = order.getTypedDataHash(
        ...
        if (
            orderInfo.orderType == LibOrder.OrderType.LIST &&
            order.royaltiesAmount > order.takerAssetAmount
        ) {
        ...
        if (
            orderInfo.orderType == LibOrder.OrderType.OFFER &&
            order.royaltiesAmount > order.makerAssetAmount
        ) {
            orderInfo.orderStatus = LibOrder.OrderStatus.INVALID_ROYALTIES;
            return orderInfo;
        }
    ...
    function _settle(
        ...
        if (protocolFeeCollector != address(0)) {
            ...
            if (
                ...
                protocolAssetData = LibAssetData.encodeERC20AssetData(
                    address(0)
                );
            } else if (protocolFeeMultiplier > 0) {
                protocolFee = buyerPayment
                    .safeMul(protocolFeeMultiplier)
                    .safeDiv(100);
            ...
            if (
                market.isActive &&
                market.feeCollector != address(0) &&
                market.feeMultiplier > 0 &&
                distributeMarketFees
            ) {
                marketFee = protocolFee.safeMul(market.feeMultiplier).safeDiv(
                    100
                );
